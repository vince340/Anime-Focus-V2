    ...
    >
      {loading ? (
        ...
        <div className="w-full flex flex-col pt-2 overflow-y-auto">
          {suggestion.map((item, index) => (
            ...
            >
              ...
              <div className="flex flex-col gap-y-[2px]">
                {item?.title && (
                  <h1 className="line-clamp-1 leading-5 font-bold text-[15px] group-hover:text-[#FFDD95]">
                    {item.title || "N/A"}
                  </h1>
                )}
                {item?.japanese_title && (
                ...
                {(item?.releaseDate || item?.showType || item?.duration) && (
                  <div className="flex gap-x-[5px] items-center w-full justify-start mt-[4px]">
                    ...
                    <p className="leading-5 text-[13px] font-medium group-hover:text-[#FFDD95]">
                      {item.showType || "N/A"}
                    </p>
                    <span className="dot"></span>
                    <p className="leading-5 text-[13px] font-light text-[#aaaaaa]">
          ...
          {!loading && hasFetched && (
            <Link
              className="w-full flex py-4 justify-center items-center bg-[#FFDD95]"
